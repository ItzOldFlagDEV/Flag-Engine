 ...
 */
// This Alphabet from Leather engine
class Alphabet extends FlxSpriteGroup
{
	...
	public var isCreditsItem:Bool = false;
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			#else
			var canDoThingyLol:Bool = (!isBold
				&& AlphaCharacter.alphabet.indexOf(character.toLowerCase()) != -1
				|| isBold
				&& AlphaCharacter.boldalphabet.indexOf(character.toLowerCase()) != -1);
			#end
			...
			new FlxTimer().start(0.05 + (0.05 * loopNum), function(tmr:FlxTimer)
			{
				if (this != null && this.active && this.visible && this.alpha != 0)
				{
					...
					var canDoThingyLol:Bool = (!isBold
						...
						|| isBold
						&& AlphaCharacter.boldalphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1
						|| isNumber
						|| isSymbol);
					#end
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			var lerpVal:Float = CoolUtil.boundTo((1.0 / FlxG.save.data.fpsCap) * 9.6, 0, 1);
